...
const { getValidVersions: _getValidVersions } = require('../packages/website/src/utils')
var configFile =
  process.env.SANDSHREW_CONFIG || path.resolve(__dirname, "../config.yml");
...
module.exports.getPelipperConfig = () => {
  return doc["pelipper"];
};
  ...
  for (const key in binaryHosts) {
    if (Object.hasOwnProperty.call(binaryHosts, key)) {
      ...
      if (!/^http/.test(mirrorPath)) {
        host = `http://${_getLocalIPv4Address()}:${
          _getAppConfig().port
        }${mirrorPath}`;
      }
...
module.exports.getValidVersions = _getValidVersions;
